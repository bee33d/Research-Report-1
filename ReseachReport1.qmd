...
This figure visually matches the descriptive table: the distribution of country-level deviation differs across cultural contexts, but overlap is substantial. In other words, context appears to matter, yet differences are moderate.
...
In the same line of testing the model's assumptions, the Shapiro-Wilk test assesses residual normality; the Breusch-Pagan test assesses homoskedasticity; and Cook’s distance checks whether a few countries dominate results. Taken together, The diagnostics indicate some heteroskedasticity (Breusch-Pagan p-value is below 0.05), so coefficient significance should be interpreted with heteroskedasticity-robust standard errors.
