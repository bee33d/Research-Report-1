...
To ensure a clearer understanding of my manipulations, let me start by explaining the design of the original study by Awad et al. The experiment varied nine moral factors across scenarios. Six factors related to the type of characters involved: age, gender, fitness, social status, species, and utilitarian. Each condition had two levels. The first compared people's preferences between saving the young or the elderly. The second choice had to do with male vs. female. The fitness factor compared preferences of saving fit vs. fat. Social status condition's two levels were high vs. low, meaning richer or poorer. Species compared between saving humans or saving pets. Three additional factors, had to do with the context. These were whether the car swerves or stays on course, whether the victims are passengers or pedestrians, and whether pedestrians are crossing legally or illegally. In our manipulation we will focus on the factors which showed the presence of some sort of preference.
...
```{r}
# Load respondent-level Moral Machine data
df <- readRDS("fullRsample.rds")
```

```{r}
#Load country-to-cultural-context lookup table
culture <- read.csv("cultures.csv")
```

```{r}
# Define the global-majority option for each scenario type so that only the conditions that showed consistent majority preference are kept.
...
# Keep one observation per respondent and scenario, based on the chosen option.
scenario_data <- df %>%
...
```{r}
# Flag whether each response deviates from the global-majority benchmark we set previously.
...
```{r}
# Clean cultural context labels since there are different values that indicate NA in the dataset
...
```{r}
# Compute scenario-level descriptives.
scenario_desc <- scenario_flagged %>%
  group_by(ScenarioTypeStrict) %>%
  summarise(
    n = n(),
    prop_against = mean(against_majority, na.rm = TRUE),
    divisiveness_index = 1 - 2 * abs(prop_against - 0.5), # rescales the distance from a 50/50 split to the unit interval.

# divisiveness_index rescales how close the split is to 50/50:
#   1 = most divisive (exactly 50/50), 0 = strongest consensus.
    .groups = "drop"
  ) %>%
...
The divisiveness index is defined as \`1 - 2 \* \|prop_against - 0.5\|\`, which rescales the distance from a 50/50 split to the unit interval. Values closer to 1 indicate a near-even split (high disagreement); values closer to 0 indicate strong consensus. This is important because it directly quantifies how polarized each scenario type is, beyond just the raw proportion.
...
This graph further illustrates the distribution of against-majority answers, with species being the least divisive scenario type (20%), and fitness being the most divisive (46,9%). This supports the claim that disagreement varies by scenario type.

### Cultural context tests (H2):

```{r}
# Aggregate to country level so each country contributes one mean proportion.
...
```{r}
# Create country x scenario cells for H3 interaction testing.
...
-   **H2 (cultural context differences):** I use a **one-way ANOVA** at country level (`prop_against ~ cultural_context`, weighted by `n`) because the outcome is a country-level proportion and the hypothesis asks whether *group means differ* across cultural contexts.
...
```{r}
# Re-compute scenario-level divisiveness for H1 reporting.
...
I use a chi‑square test of independence because both variables are categorical: scenario type and a binary indicator of deviation. The test evaluates whether the distribution of “against majority” responses is the same across scenario types. If the distributions differ, it means scenario type is associated with deviation rates.
...
We need to run an assumption check because the chi‑square test relies on an **approximation** to the true sampling distribution. That approximation is only accurate when expected counts in the contingency table are not too small. If expected counts are tiny, the chi‑square p‑value can be biased and the test may be invalid. The assumption check process is: (1) compute expected cell counts under independence; (2) confirm expected counts are not too small; (3) proceed with the chi-square approximation only if those thresholds are met.
...
In our case, the minimum expected cell count is far above 5, and there are no expected counts below 5 or 1. Therefore, the chi-square approximation is appropriate and the H1 inference is valid.

```{r}
# Standardized residuals highlight which cells deviate most from independence.
...
Standardized residuals show which cells contribute most to the association. Larger absolute residuals (Fitness) indicate scenario-type/response combinations that occur more (or less) often than expected under independence. They are essentially z-scores for each cell: `(observed − expected) / SD(expected)`. Large absolute values indicate scenario-type/response combinations that occur much more (positive) or much less (negative) than expected under independence.
...
For H2, I test whether the **mean** country-level deviation differs across cultural contexts. A one-way ANOVA answers this directly because it is designed to test whether group means differ beyond what would be expected by chance. I use weights (`n`) so that countries with larger samples contribute proportionally more information.

```{r}
fit_h2 <- lm(prop_against ~ cultural_context, data = country_data, weights = n)

# One-way ANOVA (weighted by country sample size)
fit_h2 <- aov(prop_against ~ cultural_context, data = country_data, weights = n)
h2_anova <- broom::tidy(fit_h2)
# Fit stats from lm (same model)
fit_h2_lm <- lm(prop_against ~ cultural_context, data = country_data, weights = n)
h2_glance <- broom::glance(fit_h2_lm) %>%
  transmute(
    r_squared = r.squared,
...
knitr::kable(h2_anova, digits = 4, caption = "H2 ANOVA table: cultural context differences")
```
...
With R² = 0.518 and Adj. R² = 0.371, the model fit is moderate at the country level, so cultural context accounts for a meaningful (but not exhaustive) share of cross‑country variation in deviation. The gap between R² and adjusted R² suggests caution because the sample is modest (31 countries) relative to the number of cultural categories.

The intercept is `r round(coef(fit_h2)[1], 3)`. It represents the **mean deviation** for the baseline category (Catholic). All other coefficients are differences **relative to Catholic**, so each context effect should be interpreted as a deviation from the Catholic mean.

The ANOVA F‑test is significant (F = 3.5319, p = 0.0101), so we conclude that cultural context is associated with different average deviation levels. This supports **H2** at the model level.
...
```{r}
# H2 ANOVA assumption checks

# 1) Diagnostic plots: residuals vs fitted, QQ plot, scale-location, leverage
par(mfrow = c(2,2))
plot(fit_h2)
par(mfrow = c(1,1))

# 2) Normality of residuals (ANOVA assumes normal residuals)
shapiro_p <- shapiro.test(rstandard(fit_h2))$p.value

# 3) Homogeneity of variance across groups (i'm using levene because not all countries have 2 or more observations)
library(car)
levene_p <- car::leveneTest(prop_against ~ cultural_context, data = country_data)$`Pr(>F)`[1]

# 4) Influence check (check if a few countries dominate the result)
max_cooks <- max(cooks.distance(fit_h2))

h2_diag <- tibble::tibble(
  check = c(
    "Shapiro-Wilk p-value (residual normality)",
    "Levene p-value (homogeneity of variance)",
    "Max Cook's distance"
  ),
  value = c(shapiro_p, levene_p, max_cooks)
)

knitr::kable(h2_diag, digits = 4, caption = "H2 ANOVA diagnostics")

```

These diagnostics test whether ANOVA assumptions are reasonable. The process is: (1) visually inspect residual plots for non-linearity and unequal variance, (2) test residual normality (Shapiro-Wilk), (3) test homogeneity of variance across groups (Levene), and (4) check influence (Cook’s distance).

```{r}
knitr::kable(h2_diag, digits = 4, caption = "H2 ANOVA diagnostics")
```

In the same line of testing the ANOVA assumptions, the Shapiro–Wilk test assesses residual normality and here it is satisfied because the p-value is above .05. Levene’s test assesses homogeneity of variance across cultural contexts and is satisfied because its p-value is above .05. Cook’s distance checks whether a few countries dominate results; the maximum value is small (well below conventional cutoffs like 1), so no single country appears overly influential. With these checks satisfied, the ANOVA p-values are reasonably interpretable.

### Interaction between cultural context and scenario type tests (H3):

For H3, the key question is whether scenario-type differences are the same in every cultural context. The interaction model allows scenario effects to vary by context. The ANOVA comparison between additive and interaction models is the direct test of this hypothesis. If the interaction model fits significantly better, it supports the claim that cultural differences depend on scenario type.
